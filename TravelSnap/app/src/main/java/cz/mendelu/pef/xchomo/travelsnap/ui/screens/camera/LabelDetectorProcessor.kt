...
import cz.mendelu.pef.xchomo.travelsnap.ui.screens.camera.VisionProcessorBase
...
class LabelDetectorProcessor(private val context: Context, options: ImageLabelerOptionsBase, private val viewModel: CameraScreenViewModel?, private val imageCapture: ImageCapture?) :
  ...
  override fun onSuccess(labels: List<ImageLabel>, graphicOverlay: GraphicOverlay) {
    viewModel?.processLandmarkLabels(labels.map { it.text })
    if (viewModel?.isLandmark?.value == true) {
      stop()
    }
